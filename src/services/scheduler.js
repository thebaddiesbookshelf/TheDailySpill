...
async function attemptScheduledPost(
  client,
  { recoverMissed = false } = {}
) {
  ...
  if (!dueNow && !missedToday) {
    return;
  }

  if (settings.lastScheduledDate === local.date) {
    return;
  }
  ...
  try {
    ...
    console.log(
      `☕ Scheduled Edition #${String(result.edition).padStart(3, '0')} posted to #${result.channel.name}.`
    );
  } catch (error) {
    console.error(
      'Daily Spill scheduled post failed:',
      error.message || error
    );
  } finally {
    running = false;
